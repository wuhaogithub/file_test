...
class BaseJob(Base, LoggingMixin):
    ...
    and end time.
    要派生的作业的抽象类。作业是处理具有状态和持续时间的项，而不是任务实例。
    例如，BackfillJob是一个任务实例运行的集合，但是应该有自己的状态、开始和结束时间。
    """
    ...
    def heartbeat(self):
        ...
        sleep at all.
        Heartbeats使用latest_heartbeat的时间戳更新数据库中的作业条目，并允许在外部终止作业。
        这允许在系统级监视实际活动的内容。例如，SchedulerJob的旧心跳表示出了问题。
        这还允许在外部终止任何作业，不管谁在运行它，也不管它在哪个机器上运行。
        注意，如果你的心跳被设置为60秒，并且从上一次心跳开始，经过10秒的处理后调用此方法，则它将休眠50秒以完成60秒，并保持稳定的心率。
        如果你在调用它之前超过60秒，它根本不会睡觉。
        """
...
class BackfillJob(BaseJob):
    ...
    回填作业由特定时间范围内的dag或subdag组成。
    它以正确的顺序触发一组任务实例运行，并持续到完成一组任务实例所需的时间。
    """
    ID_PREFIX = 'backfill_'
    ...
    class _DagRunTaskStatus(object):
        ...
        回填工作的内部状态。这个类只打算在BackfillJob实例中实例化，它将跟踪任务的执行，例如。跑步，跳过，成功，失败，等等。
        与回填作业相关的dag运行信息也在这个结构中被跟踪，例如。成品等。
        与dag运行/任务的执行相关的任何其他状态信息都可以包含在这个结构中，因为这样更容易传递。
        """
