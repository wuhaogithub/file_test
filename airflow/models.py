...
class DagBag(BaseDagBag, LoggingMixin):
    ...
    dagbag是dags的集合，从文件夹树中解析出来，具有高级配置设置，
    比如使用什么数据库作为后端，使用什么执行者来触发任务。
    这使得为生产和开发、测试或不同的团队或安全配置文件运行不同的环境变得更加容易。
    原本的系统级别设置现在变成了dagbag级别，这样一个系统可以运行多个独立的设置集。
...
class Connection(Base, LoggingMixin):
    ...
    占位符，用于存储关于不同数据库实例连接信息的信息。
    这里的思想是，脚本在使用操作符或挂钩时使用对数据库实例的引用(conn_id)，
    而不是硬编码主机名、登录和密码。
    """
...
class DagPickle(Base):
    ...
    dag可以来自不同的地方(用户repos、主repo等)，
    也可以在不同的地方(不同的执行器)执行。此对象表示DAG的一个版本，并成为BackfillJob执行的真实来源。
    pickle是一个本机python序列化对象，在本例中，它在作业期间存储在数据库中。
    The executors pick up the DagPickle id and read the dag definition from
    the database.
    执行器获取DagPickle id并从数据库中读取dag定义。
    """
    id = Column(Integer, primary_key=True)
...
class TaskInstance(Base, LoggingMixin):
    ...
    任务实例存储任务实例的状态。此表是关于运行了哪些任务以及它们所处的状态的权威和单一事实来源。
    SqlAlchemy模型没有任务或dag模型的SqlAlchemy外键，这是为了更好地控制事务。
    此表上的数据库事务应该确保双触发器和关于任务实例是什么或不准备运行的任何混淆，即使多个调度器可能正在触发任务实例。
    """
...
class BaseOperator(LoggingMixin):
    ...
    这个类是抽象的，不应该实例化。实例化一个派生自这个类的类会创建一个task对象，
    该对象最终成为DAG对象中的一个节点。任务依赖项应该通过使用set_upstream和/或set_方法来设置。
...
class DAG(BaseDag, LoggingMixin):
    ...
    dag(有向无环图)是具有方向依赖关系的任务集合。dag也有一个进度表，一个开始日期和一个结束日期(可选)。
    对于每个调度(例如每天或每小时)，DAG需要在满足其依赖项时运行每个单独的任务。
    某些任务具有依赖于其自身过去的特性，这意味着它们在完成之前的调度(以及上游任务)之前不能运行。

    DAGs essentially act as namespaces for tasks. A task_id can only be
    added once to a DAG.
    dag本质上充当任务的名称空间。task_id只能被添加到DAG中一次。

    :param dag_id: The id of the DAG
    :type dag_id: string
    :param description: The description for the DAG to e.g. be shown on the webserver
    :type description: stringazsx
